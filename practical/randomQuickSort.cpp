...
// Random Quick Sort processor
int paritition(int arr[], int high, int low = 0)
{
    int i = low;
    int j = high;
    int pivot = rand() % (high - low + 1) + low; // Random pivot within range of high and low
...
void randomQuickSort(int arr[], int high, int low = 0)
{
    int q;
    if (low < high)
    {
        q = paritition(arr, high, low);
        randomQuickSort(arr, q - 1, low);
        randomQuickSort(arr, high, q + 1);
    }
}
